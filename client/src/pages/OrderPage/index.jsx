...
const OrderPage = () => {
  ...
  const orderByUserQuery = useQuery({
    queryKey: ['orderByUserQuery', user?.user_id, orderType],
    queryFn: () => getAllOrderByUser(user?.user_id, orderType ? { order_status: orderType } : {}),
          ...
          <>
            {orderByUserQuery.data.length > 0 ? (
              orderByUserQuery.data.reverse().map((order) => (
                <OrderItem key={order.id} data={order} updateData={updateData} />
              ))
            ) : (
