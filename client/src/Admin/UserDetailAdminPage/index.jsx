            ...
            {/* <div className="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                ...
                <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <form action='' method='POST' className=" text-[#100e3d] relative text-center transform rounded-lg bg-white shadow-xl transition-all p-5 flex flex-col">
                            <span className=' uppercase font-semibold mb-5'>Update status</span>
                            <select className='w-[200px] h-[40px] rounded-[5px] outline-none border-[2px] border-[#100e3d]'>
                            ...
                            <span className='cursor-pointer text-center rounded-[5px] text-[#100e3d] mt-2'>Cancel</span>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-6 gap-x-4">
                <div className="col-span-2 bg-white rounded-xl overflow-hidden p-4 shadow-xl">
                    ...
                    <div className="mt-16 flex flex-col items-start">
                        ...
                        <span className="text-gray-500">#ID: {data?.user_id}</span>
                        ...
                        <div className="mt-5 flex flex-col gap-y-3">
                            ...
                            {(Number(userId) !== user?.user_id) &&
                                <button className='w-[120px] h-[45px] bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-all duration-200'>Set role</button>}
                        </div>
                    </div>
                </div>
                            ...
                            {orderByUser?.map((item, index) => {
                                return (
                                    <tr key={index} className='hover:bg-gray-50 transition-all'>
                                        <td className='p-4 text-sm text-gray-500'>
                                            <div className="flex items-center">
                                                ...
                                                <div className="ml-3">
                                                    ...
                                                    <div className="text-gray-400">ODate: {helpers.dateFormat(item?.order_date)}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className='p-4 text-sm text-gray-500'>
                                        ...
                                        <td className='p-4 text-md text-gray-700 font-medium'>{helpers.numberFormat(item?.total_amount)} VND</td>
                                        <td className='p-4 text-sm text-gray-500'>
