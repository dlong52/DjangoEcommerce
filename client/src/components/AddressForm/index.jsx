...
const AddressForm = ({ showAddressForm }) => {
    ...
    return (
        <div className='fixed inset-0 z-50 transition-all duration-500'>
            ...
            <form className='bg-white rounded-[5px] w-fit top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute p-5' action="">
                <h1 className='text-[20px] font-semibold pb-3'>Địa chỉ mới</h1>
                <div className="grid grid-cols-12 gap-4">
                    <input
                        type="text"
                        placeholder='Họ và tên'
                        className="rounded-[5px] outline-none pl-3 h-[45px] shadow-input col-span-6"
                        value={detailAddress}
                    ...
                    <div className="col-span-12 flex gap-2 items-center">
                        <div className="w-[18px] overflow-hidden flex items-center justify-center aspect-square rounded-[5px] border-2 border-gray-500">
                            {isDefaultAddress && <FontAwesomeIcon className='h-full aspect-square text-[10px] bg-main text-white' icon={faCheck} />}
                        </div>
                        <span
                            className='text-[16px] cursor-pointer select-none'
                            onClick={() => { setDefaultAddress(!isDefaultAddress) }}
                    ...
                    <div className="col-span-12 flex gap-x-4 items-center justify-end">
                        ...
                        <button type='button' className="py-2 px-4 bg-main text-white rounded-[5px]">Hoàn thành</button>
                    </div>
                </div>
            </form>
