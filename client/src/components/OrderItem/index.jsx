  ...
  return (
    <div className="bg-white border shadow-inner rounded-xl p-6 mb-6 transition">
      <div className="flex justify-between items-center border-b pb-3 mb-3">
        <span className="text-gray-500 text-sm">Ngày đặt: {helpers.dateFormat(data?.order_date)}</span>
        ...
        {data?.order_details?.map((item, index) => (
          <div className="flex justify-between items-start border-b pb-3" key={index}>
            <div className="flex gap-5 items-center">
              <img className="w-24 h-24 object-cover rounded-lg shadow-sm" src={item?.image} alt={item?.product_name} />
              <div className="flex flex-col">
                <a href={`/shop/${item?.product}`} className="text-lg font-medium text-gray-900">{item?.product_name}</a>
      ...
      <div className="mt-5 flex justify-between">
        <span className="text-lg font-semibold text-gray-900">Thành tiền: {helpers.numberFormat(data?.total_amount).toLocaleString()} VND</span>
        <div className="flex gap-4">
          ...
          <a href={`/profile/order/${data?.order_id}`} className="text-gray-600 border border-gray-400 px-5 py-3 rounded-lg hover:border-gray-600 hover:text-gray-800 transition">
            Chi tiết đơn hàng
          </a>
        </div>
      </div>
