...
const addToCart = async (productId, quantity, size, user_id) => {
    try {
        const response = await axios.post(`${import.meta.env.VITE_API}/cart/add-to-cart`, { 
            user_id: user_id,
...
const getAllCartItems = async (user_id) => {
    try {
        const res = await axios.get(`${import.meta.env.VITE_API}/cart/get-all/${user_id}`);
        return res.data;
    } catch (error) {
...
const updateCartItem = async (cart_item_id, quantity, size) => {
    try {
        const res = await axios.put(`${import.meta.env.VITE_API}/cart/update/${cart_item_id}`, {
            quantity: quantity,
            size: size
        });
        return res.data;
